"""
Script d'exécution de la méthode SLIC_IPOL
"""
import sys
import os
...
def run_slic_ipol_single_image(image_path, n_segments=200, compactness=10.0, 
                         max_iter=10, save_results=True):
    """
    Exécute SLIC_IPOL sur une seule image.
    ...
    # Appliquer SLIC_IPOL
    print("Application de SLIC_IPOL...")
    slic = SLIC_IPOL(n_segments=n_segments, compactness=compactness, max_iter=max_iter)
    ...
    fig = visualize_segmentation(image, labels, 
                             title=f"SLIC_IPOL (n={n_segments}, m={compactness})",
                             show_boundaries=True, boundary_color=(1, 1, 0))
    
    # Sauvegarder
    if save_results:
        results_dir = Path('results/slic_ipol')
...
def run_slic_batch(image_dir, n_segments=200, compactness=10.0, max_iter=10):
    """
    Exécute SLIC_IPOL sur un dossier d'images.
    ...
    for img_path in image_paths:
        print(f"\nTraitement: {img_path}")
        labels, metrics = run_slic_ipol_single_image(
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Exécution SLIC_IPOL')
    ...
    if args.image:
        run_slic_ipol_single_image(args.image, args.n_segments, 
        ...
        print("python run_slic_ipol.py --image path/to/image.jpg --n_segments 300 --compactness 15")
